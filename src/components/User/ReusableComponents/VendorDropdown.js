...
const VendorDropdown = ({ vendors, onSelectVendor, initialVendorName }) => {
  ...
  const handleVendorSearch = (value) => {
    console.log('handleVendorSearch:', { value, vendors: vendors.length });
  ...
  const handleVendorKeyDown = (e) => {
    console.log('handleVendorKeyDown:', { key: e.key, highlightedIndex, vendorDropdownVisible });
    if (!vendorDropdownVisible) {
      if (e.key === 'Enter' || e.key === 'ArrowDown') {
        setVendorDropdownVisible(true);
        setHighlightedIndex(filteredVendors.length > 0 ? 0 : -1);
        e.preventDefault();
      }
      return;
    }
